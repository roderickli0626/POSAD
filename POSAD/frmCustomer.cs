...
namespace POSAD
{
    public partial class frmCustomer : Form
    {
        ...
        SqlDataReader dr;
        public clsCustomer cus = new clsCustomer();
        ...
        {
            frmCustomerModule supplierModule = new frmCustomerModule(this);
        ...
        private void dgvSupplier_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Use this user for Invoice", "CONFIRM", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    foreach (DataGridViewRow row in dgvSupplier.SelectedRows)
                    {
                       

                        cus.Customer = row.Cells[2].Value.ToString();
                        cus.Address = row.Cells[3].Value.ToString();
                        cus.contactPerson = row.Cells[4].Value.ToString();
                        cus.Phone = row.Cells[5].Value.ToString();
                        cus.Email = row.Cells[6].Value.ToString();
                        cus.Fax = row.Cells[7].Value.ToString();
                    }
